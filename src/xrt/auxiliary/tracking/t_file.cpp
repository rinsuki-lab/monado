...
t_camera_calibration_load_v2(cJSON *cjson_cam, t_camera_calibration *cc)
{
	...
	CALIB_ASSERTR(model_map.count(model) > 0, "Unknown camera model: '%s'", model.c_str());
	cc->distortion_model = model_map.at(model);
	...
	case T_DISTORTION_FISHEYE_KB4:
		...
		break;
	default: CALIB_ASSERTR(0, "BUG: Unhandled distortion model %d", cc->distortion_model); break;
	}
...
t_stereo_camera_calibration_from_json_v2(cJSON *cjson, struct t_stereo_camera_calibration **out_stereo)
{
	...
	CALIB_ASSERTR(cameras.size() == camera_len, "%lu camera(s) must be specified, %zu given", camera_len,
	              cameras.size());
