...
XRAPI_ATTR XrResult XRAPI_CALL
oxr_xrSetInputDeviceStateBoolEXT(XrSession session, XrPath topLevelPath, XrPath bindingPath, XrBool32 state)
{
	OXR_TRACE_MARKER();
	...
	struct oxr_instance *inst = sess->sys->inst;
	OXR_VERIFY_INSTANCE_PATH(&log, inst, topLevelPath);
	OXR_VERIFY_INSTANCE_PATH(&log, inst, bindingPath);

	return oxr_automation_set_input_device_state_boolean(&log, sess, topLevelPath, bindingPath, state);
}

XRAPI_ATTR XrResult XRAPI_CALL
oxr_xrSetInputDeviceStateFloatEXT(XrSession session, XrPath topLevelPath, XrPath bindingPath, float state)
{
	OXR_TRACE_MARKER();
	...
	return oxr_automation_set_input_device_state_float(&log, sess, topLevelPath, bindingPath, state);
}

XRAPI_ATTR XrResult XRAPI_CALL
oxr_xrSetInputDeviceStateVector2fEXT(XrSession session, XrPath topLevelPath, XrPath bindingPath, XrVector2f state)
{
	OXR_TRACE_MARKER();
	...
	return oxr_automation_set_input_device_state_vec2(&log, sess, topLevelPath, bindingPath, state);
}

XRAPI_ATTR XrResult XRAPI_CALL
oxr_xrSetInputDeviceLocationEXT(XrSession session, XrPath topLevelPath, XrPath bindingPath, XrSpace space, XrPosef pose)
{
	OXR_TRACE_MARKER();
	...
	static_assert(sizeof(struct xrt_pose) == sizeof(XrPosef), "pose size mismatch");
	struct xrt_pose p = {0};
	memcpy(&p, &pose, sizeof(p));

	return oxr_automation_set_input_device_location(&log, sess, topLevelPath, bindingPath, spc, p);
}
