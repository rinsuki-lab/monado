...
automated_device(struct xrt_device *xdev)
{
...
{
	struct automated_device *dev = automated_device(xdev);
...
automated_device_create(struct oxr_interaction_profile *profile, enum xrt_device_type type)
{
	...
	struct automated_device *dev = U_DEVICE_ALLOCATE(struct automated_device, flags, profile->binding_count, 0);
	if (dev == NULL) {
		return NULL;
	}
	...
	dev->base.update_inputs = u_device_noop_update_inputs;
	...
	snprintf(dev->base.tracking_origin->name, XRT_TRACKING_NAME_LEN, "%s automated tracker",
...
oxr_automation_device_is_automated(struct xrt_device *xdev)
{
	...
	return xdev->destroy == automated_device_destroy;
}
    ...
    struct oxr_logger *log, struct oxr_session *sess, XrPath interactionProfile, XrPath topLevelPath, XrBool32 isActive)
{
	...
	// Find the device index for an automated device of the desired type.
	size_t idx;
	for (idx = 0; idx < xsysd->xdev_count; idx++) {
		struct xrt_device *tmp = xsysd->xdevs[idx];
	...
	if (xdev == NULL) {
		...
		if (xdev == NULL) {
			return oxr_error(log, XR_ERROR_RUNTIME_FAILURE, "failed to allocate automated device");
		}
	...
	// Update action attachments according to the top-level /user path supplied
                            ...
                            struct xrt_space_relation *out_relation)
{
	...
	struct automated_device *dev = automated_device(space->action.xdev);
